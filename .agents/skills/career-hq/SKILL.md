...
Read the full current posting from a credible source before adding it. Save the verbatim posting text to a temporary local file, then run `add-job`. Require current-listing and credible-source confirmation. Write `--job-summary` as a source-grounded, plain-language summary of 1–3 short sentences that leads with what the role actually is and what the person would do day to day. When the posting contains sections titled **What you'll do**, **What you will do**, **Job responsibilities**, **Key responsibilities**, **Duties**, **Your impact**, **Day-to-day**, or close variants, use those sections as the primary summary source ahead of general role overviews or company descriptions. The summary may be followed by short Markdown bullet points when a responsibility is clearer as a discrete item than inside the prose summary. Include only bullets that add useful new information; never repeat or lightly rephrase a responsibility already covered in the sentences. Show that summary first, then fit, strongest match, largest gap, major risk, compensation, location/work arrangement, and one next action. Missing preferred qualifications alone do not disqualify a role.
...
Every generated resume must include the exact saved application role title in the header, centered directly beneath the applicant's name and above the contact block in both DOCX and PDF. Treat a missing role title as a generation error; never omit it, infer a replacement, or substitute a broader career label.

Before drafting, inspect the applicant's verified baseline resume sources and `resumePreferences.layoutProfile`. A verified layout profile is an explicit user design choice and overrides the baseline visual reference without deleting it. For a selected supported profile, generate both formats only through `prepare-resume`; do not create an application-specific builder or a bespoke document layout. Record the generator and layout-profile version in the evidence manifest. When no explicit layout profile exists, use the verified baseline visual system as the design authority: preserve its recognizable page count, typography, margins, title and contact treatment, section hierarchy, spacing, bullet style, color use, and overall density unless the user asks for a different design.

Use the first page deliberately. Put every relevant role under **Professional Experience** on page one when they fit legibly together. Never leave page one half empty because of a manual page break. If the roles do not fit together, reorder supporting sections such as skills, education, or projects before reducing readability; preserve the selected layout profile and allow a clean second page rather than forcing smaller text.
